    ...
    public class SearchService : ISearchService
    {
            ...
            ClaimsPrincipal user, long personId)
        {
            ...
            if (searchAll || request.Types.Contains(SearchEntityType.Blog))
                results.AddRange(await _blogSearch.SearchAsync(request.Query, user, personId));

            if (searchAll || request.Types.Contains(SearchEntityType.Tour))
                results.AddRange(await _tourSearch.SearchAsync(request.Query, user, personId));

            if (searchAll || request.Types.Contains(SearchEntityType.User))
                results.AddRange(await _userSearch.SearchAsync(request.Query, user, personId));
